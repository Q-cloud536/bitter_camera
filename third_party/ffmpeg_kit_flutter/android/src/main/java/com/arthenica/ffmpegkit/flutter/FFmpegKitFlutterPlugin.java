...
public class FFmpegKitFlutterPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler, EventChannel.StreamHandler, PluginRegistry.ActivityResultListener {
    ...
    // NOTE:
    // Flutter's old (v1) Android embedding used PluginRegistry.Registrar and a static registerWith.
    // Newer Flutter/Android embedding toolchains removed that API, so keeping it causes compilation
    // errors like "cannot find symbol: class Registrar".
    //
    // This plugin is already v2-embedding-capable via FlutterPlugin + ActivityAware; we rely on that
    // and intentionally drop v1 registration.
